class Obstacles {
  ...
  reset() {
    this.akatsuki.forEach((p) => p.remove());
    this.akatsuki = [];
  }
  ...
  isCollision(player, obstacle) {
    ...
    const xCollision =
      obstacleClientRect.right > playerClientRect.left &&
      obstacleClientRect.left < playerClientRect.right;
    const yCollision =
      playerClientRect.bottom > obstacleClientRect.top &&
      playerClientRect.top < obstacleClientRect.bottom;
